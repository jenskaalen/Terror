...
public class VLight : MonoBehaviour
{
...
#else
		spotNear = camera.near;
		spotRange = camera.far;
		spotAngle = camera.fov;
#endif		
	}
		...
		if(createdMaterial)
		{
			switch(lightType)
			{
				case LightTypes.Point:
					...
					{
						renderer.sharedMaterial.SetTexture("_LightColorEmission", pointEmission);
					}
					if(pointNoise != null)
					{
						renderer.sharedMaterial.SetTexture("_NoiseTex", pointNoise);
					}
					if(pointShadow != null)
					{
						renderer.sharedMaterial.SetTexture("_ShadowTexture", pointShadow);
					}
					break;
				case LightTypes.Spot:
					...
					{
						renderer.sharedMaterial.SetTexture("_LightColorEmission", spotEmission);
					}
					if(spotNoise != null)
					{
						renderer.sharedMaterial.SetTexture("_NoiseTex", spotNoise);
					}
					if(spotShadow != null)
					{
						renderer.sharedMaterial.SetTexture("_ShadowTexture", spotShadow);
					}
					break;
			}
	...
	private void CleanMaterials()
	{
		...
		SafeDestroy(renderer.sharedMaterial);
		SafeDestroy(meshContainer);
	...
	private Matrix4x4 CalculateProjectionMatrix()
	{
		...
		{
			float halfOrtho = camera.orthographicSize * 0.5f;
	...
	private void BuildMesh(bool manualPositioning, int planeCount, Vector3 minBounds, Vector3 maxBounds)
	{
		...
		for(int i = 0; i < planeCount; i++)
		{
			...
			{
				Plane[] planes = GeometryUtility.CalculateFrustumPlanes(_projectionMatrixCached * camera.worldToCameraMatrix);
			...
			case ShadowMode.Realtime:
				if(SystemInfo.supportsImageEffects)
				{
					...
					if(layer != -1)
					{
						...
						{
							case LightTypes.Spot:
								camera.targetTexture = _depthTexture;
								camera.projectionMatrix = CalculateProjectionMatrix();
								camera.RenderWithShader(renderDepthShader, "RenderType");

								break;
							case LightTypes.Point:
								camera.projectionMatrix = Matrix4x4.Perspective(90, camera.aspect, 0.1f, far / 2f);
								camera.SetReplacementShader(renderDepthShader, "RenderType");
								camera.RenderToCubemap(_depthTexture, 63);
	...
	private bool CameraHasBeenUpdated()
	{
		...
		hasBeenUpdated |= camera.orthographicSize != _prevOrthoSize;
		hasBeenUpdated |= camera.isOrthoGraphic != _prevIsOrtho;
	...
	public void UpdateLightMatrices()
	{
		...
		_worldToCamera = camera.worldToCameraMatrix;
	...
	public void SetShaderPropertiesBlock(MaterialPropertyBlock propertyBlock)
	{
		...
		propertyBlock.AddVector(_idLightParams, new Vector4(near, far, far - near, (camera.isOrthoGraphic) ? Mathf.PI : fov * 0.5f * Mathf.Deg2Rad));
	}
	...
	public void SetShaderPropertiesMaterials()
	{
		...
		float fov = camera.fov;
#endif
		material.SetVector("_LightParams", new Vector4(near, far, far - near, (camera.isOrthoGraphic) ? Mathf.PI : fov * 0.5f * Mathf.Deg2Rad));
	}
